...
2012年TRI-TAC项目被JNMS(Joint Network Management System)项目取代。
...
- 计算机网络栅格由各种计算机、存储器、网格软件平台、数据库、地理信息系统等计算信息设施组成。海量的信息往往使参战人员无所适从，难以决策，计算机网络栅格的作用就是对搜集到的信息进行筛选、分析、处理，区分轻、重、缓、急，并成实时或近实时的战场态势图。利用计算机网络栅格，美军还可计算出最佳路线，选择最恰当的作战目标，采取最有效的作战方法和手段，动用最合适的力量，最大限度地打击敌人、减少己方损失。因此，计算机网络栅格的作用，用一句话概括，即处理信息以获取决策优势。
